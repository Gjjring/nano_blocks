...
dash.register_page(__name__, path='/page_6')
...
layout = html.Div([
    ...
    html.Div(id='result-box', children=[
        dcc.Loading(
            ...
            children=[
                html.Div(
                    html.Button("",id='transmission-btn', disabled=True),
                    style={
                        "display": "flex",
...
def run_jcmwave_simulation(threshold_data, poly_data, project, port):
    ...
    for polygon in poly_data:
        ...
        new_poly = np.zeros_like(polygon)
        new_poly[:, 0] = (polygon[:, 0]- (half_buffer+1) )/ (image_width_without_buffer-1) * keys['user_area_width'] + 1 - keys['cd_width']/2
        new_poly[:, 1] = (polygon[:, 1]- (half_buffer+1) )/ (image_height_without_buffer-1) * keys['user_area_height'] + 0.75 - keys['cd_height']/2
        resized_polygons.append(new_poly)

    keys['polygons'] = list(zip(resized_polygons, nesting_levels.values()))
    ...
    if 'simulation_hash' in session and hash_value == session['simulation_hash']:
        ...
        if not project == "jcmwave":
            fieldbag_source = os.path.join(first_dir, "1D", "project_results", "fieldbag.jcm")
            if not os.path.isfile(fieldbag_source):
...
def swap_displayed_data(plot_type):
    if plot_type == "Simulation Mesh":
        return {"height": "480px", "width": "640px", "display": "block"}, {"height": "640px", "width": "640px", "display": "none"}, {"height": "480px", "width": "640px", "display": "none"}
